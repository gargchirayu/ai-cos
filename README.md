...
- New messages are manually loaded into `messages.json` and there is no background polling or webhook to capture incoming messages automatically
- The CEO's identity and team roles are inferred from message content; no org chart or contact directory is provided
- Drafted responses are suggestions for the CEO to review before sending, and the system never sends anything automatically
...
- No authentication as this is a single-user local tool, meant for the CEO only; the API key is also read from the environment directly. In a prodction-ready build, there should be an authentication for the user and the LLM API call.
