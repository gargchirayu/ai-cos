import { ChevronDown, ChevronRight, CornerDownRight, History, UserRound } from "lucide-react";
    ...
    <div className={`rounded-xl border bg-white shadow-sm ${u.ring}`}>
      <div className="p-4">
        ...
        {situation.drafted_response && (
          <div className="mt-2.5 rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <CopyButton text={situation.drafted_response} />
            </div>
        ...
        >
          ...
          {open ? "Hide" : "Show"} the thread ({thread.length} message
          {thread.length === 1 ? "" : "s"})
        </button>
      </div>
                ...
                <li key={m.id} className="flex gap-2.5">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5 text-[12px]">
                      ...
                      <span className="text-slate-400">{formatTime(m.timestamp)}</span>
                      <StatusBadge status={m.status} />
                      {m.superseded_by && (
                        <span className="flex items-center gap-0.5 text-[11px] text-slate-400">
                          <CornerDownRight size={11} /> see #{m.superseded_by}
                        </span>
                      )}
                    </div>
                    <p className="text-[12.5px] leading-relaxed text-slate-500">{m.reasoning}</p>
                  </div>
                </li>
              );
            })}
